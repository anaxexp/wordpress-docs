...
3. Download `traefik.yml` file (inside of `wordpress4docker.tar.gz` archive) from the [latest stable release](https://github.com/anaxexp/wordpress4docker/releases) to the parent dir where _site1_ and _site2_ dirs are
...
This set up also works for other Docker projects (non PHP and non wordpress4docker based). You can replace nginx-proxy config with Traefik and get other projects all routed with on traefik container.
...
To report issues about multi-project set up with wordpress4docker or if you have more suggestions and use-cases, please submit an issue on GitHub.
